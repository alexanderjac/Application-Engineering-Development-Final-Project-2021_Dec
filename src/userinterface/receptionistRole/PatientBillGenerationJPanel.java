...
public class PatientBillGenerationJPanel extends javax.swing.JPanel {
    ...
    private ReceptionistOrganisation organization;
    private Enterprise enterprise;
    private UserAccount userAccount;
    private EcoSystem eco;
    private Network network;

    public PatientBillGenerationJPanel(JPanel userProcessContainer, UserAccount account, ReceptionistOrganisation organization, Enterprise enterprise, Network network, EcoSystem eco) {
        initComponents();
...
//        for (UserAccount user : organization.getUserAccountDirectory().getUserAccountList()) {
        for (UserAccount user : enterprise.getUserAccountDirectory().getListOfUserAccounts()) {
            System.out.println(user.getWorkQueue().getListOfWorkRequests());
    ...
    private void btnGenerateBillActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGenerateBillActionPerformed
        ...
        if (work.getHospitalFee() == 0) {
            ...
            request.setDetailsOfPatient(work);
            request.setRequest(work);
            System.out.println(work);
            request.setSender(userAccount);
            request.setObjectName(work.getPatientFirstName()+" "+work.getPatientLastLame());
            request.setAddress(enterprise.getOrganizationName());
            request.setStatus("Pending payment");
            request.setMessage("Pay Bills");
            request.setTypeOfRequest("HospitalFee");
            request.setPatientFee(work.getHospitalFee());
            for (Enterprise enterp : network.getEnterpriseDirectory().getListOfEnterprises()) {
                if (enterp.getEnterpriseType().equalsIgnoreCase(Enterprise.EnterpriseType.FundRaiser.getValue())) {
                    for (UserAccount userAcc : enterp.getUserAccountDirectory().getListOfUserAccounts()) {
                        if (userAcc.getRole() instanceof AdminRole) {
                            userAcc.getWorkQueue().getListOfWorkRequests().add(request);
