...
public class ManageShelterRequestJPanel extends javax.swing.JPanel {
    ...
    public ManageShelterRequestJPanel(JPanel userProcessContainer, UserAccount account, NGOOrganisation organisation, Enterprise enterprise, Network network, EcoSystem business) {
    ...
    private void btnAssignShelterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAssignShelterActionPerformed
        ...
        int age = request.getAgeOfPerson();
    ...
    private void populateTable() {
        ...
        if (enterprise.enterpriseType.equalsIgnoreCase(Enterprise.EnterpriseType.NGO.getValue())) {
             for (UserAccount HASUser : enterprise.getUserAccountDirectory().getListOfUserAccounts()) {
            if (HASUser.getRole() instanceof AdminRole) {
                                        System.out.println("check20 "+HASUser.getUsername());
                for (WorkRequest request : HASUser.getWorkQueue().getListOfWorkRequests()) {
                    if ((request.getStatus().equalsIgnoreCase("Awaiting for Shelter"))|(request.getStatus().equalsIgnoreCase("Shelter Requested")) |(request.getStatus().equalsIgnoreCase("Awaiting for Shelter and Treatment"))) {
                        System.out.println("check20");
                        Object[] row = new Object[4];
                        row[0] = request;
                        row[1] = request.getAssignedShelter();
                        row[2] = request.getStatus();
                        row[3] = request.getAgeOfPerson();
                        model.addRow(row);
                    }
                }
            }
       ...
       DefaultTableModel model1 = (DefaultTableModel) tblShelter.getModel();
        ...
        List<Organization> org =  enterprise.getOrganizationDirectory().getListOfOrganizations();
